...
def product_list(request):
    ...
    data        = Product.objects.all().order_by('-id')[:2]
...
def my_dashboard(request):
    ...
    return render(request, 'user/dashboard.html', {'orders':orders, 'monthNumber':monthNumber, 'totalOrders':totalOrders})
...
def add_wishlist(request):
    ...
    user        = request.user
    data = {}
    checkw      = WhishList.objects.filter(user=user, product=product).count()
    if checkw > 0:
        data = {
            'bool': False
        }
...
def my_wishlist(request):
    wlist  = WhishList.objects.filter(user=request.user).order_by('id')
...
def save_address(request):
    msg = None
    if request.method == 'POST':
        form = AddressBookAdd(request.POST)
        if form.is_valid():
...
def activate_address(request):
    p_id    = str(request.GET['id'])
    UserAddressBook.objects.update(status=False)
    UserAddressBook.objects.filter(id=p_id).update(status=True)
...
def update_address(request, id):
    ...
    msg     = None
    if request.method == 'POST':
        form = AddressBookAdd(request.POST, instance=address)
        if form.is_valid():
            saveForm = form.save(commit=False)
            saveForm.user = request.user
            if 'status' in request.POST:
    ...
    form = AddressBookAdd(instance=address)
